...
filename = "freddy_gray.png"
shortfile = "ABSTestingFred"
InputFolder = "Input/"
OutputFolder = "Output/"
WorkingFolder = "Workingdir/"
plotmatlab = True #Set to True to create a gif of the scanning process 
...
class AMatrix:
    ...
    def CreateCurvedFanRays(self, sourceloc=None, detectors=None, fan_angle_degrees=60):
        """
        sid = source-to-isocenter distance (distance from image center to source)
        sdd = source-to-detector distance (radius of curved detector arc around the source)
        fan_angle_degrees = total angular span of detector arc
        """
        w = self.ReconstructionWidth
        c = self.center
        ...
        half = math.radians(fan_angle_degrees / 2.0)
        ...
        for a in angles:
            ...
            det = point(
                source.x - detectors * math.cos(a),
                source.y + detectors * math.sin(a),
                0
            )
    
            self.Rays.append(
                Ray(point1=point(source.x, source.y, 0),
                    point2=det,
                    rotationOrigin=c)
            )
...
def main():
    ...
    if plotmatlab: #Change at top to enable or disable
        ...
        for i in range(0,181):#Debug block
            ax.imshow(img2)
            margin = 10
            w = ScanningResult.ReconstructionWidth

            ax.set_xlim([-margin, w + margin])
            ax.set_ylim([w + margin, -margin])  # flips y so image isn’t upside down
